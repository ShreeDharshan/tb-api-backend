...
from report_logic import router as report_router   # << only the router now
...
def extract_jwt_user_info(jwt_token: str) -> dict:
    """
    Decode the JWT payload (no signature verification) and return a dict.
    ...
    try:
        ...
        logger.warning(f"[JWT] Failed to parse token payload: {e}")
        return {}
...
def get_my_devices(authorization: str = Header(...)):
    ...
    user_info = extract_jwt_user_info(jwt_token) or {}
    authority = user_info.get("authority", "")
    customer = user_info.get("customerId") or {}
    customer_id = customer.get("id", "")

    if not authority:
        raise HTTPException(status_code=401, detail="Invalid or unparseable JWT")

    headers = {"X-Authorization": f"Bearer {jwt_token}"}

    if authority == "CUSTOMER_USER":
        if not customer_id:
            raise HTTPException(status_code=403, detail="Missing customerId in token")
...
def admin_jwt_status():
    # NOTE: thingsboard_auth.get_admin_jwt(account_id, base_url) signature
    token = get_admin_jwt("account1", TB_HOST)
    if token:
