"""
pack_format.py — Common parser for ThingsBoard 'pack_raw' strings.
...
Format:
    ...
    - Known keys are coerced to int or float; others remain strings.

Core helpers exported:
    - parse_pack_raw(s: str, *, int_keys=None, float_keys=None, lowercase_keys=False) -> dict
    - ts_seconds(parsed: dict, default: int | None = None) -> int | None
    - ts_millis(parsed: dict, default: int | None = None) -> int | None
    - door_to_bit(val: object) -> int | None   # "OPEN"/"CLOSED"/1/0 -> 1/0/None
    - get_int(parsed: dict, key: str, default=None)
    - get_float(parsed: dict, key: str, default=None)

Default known numeric keys can be extended by callers via function params.

Example:
    s = "v=1|ts=1724051405|laser_val=15758|accel_x_val=0.06|door_val=OPEN"
    d = parse_pack_raw(s)
    # d == {'v': 1, 'ts': 1724051405, 'laser_val': 15758.0, 'accel_x_val': 0.06, 'door_val': 'OPEN'}
    ms = ts_millis(d)        # 1724051405000
    door = door_to_bit(d.get('door_val'))  # 1
"""
