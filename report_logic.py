...
# Make sure pack_format.py is present (we already wrote this in earlier step)
...
def _extract_from_pack_calc(pack: str, want: List[str]) -> Dict[str, Any]:
    """
    Parse one pack_calc row and return dict of wanted fields.
    pack_calc expected to include: h, fi, fl, dir, st (and ts inside).
    """
...
def generate_report(
    ...
    - Parses 'pack_calc' (and 'pack_raw' when needed).
    ...
    # Build time window in ms (full days inclusive)
    ...
    if need_calc:
        for p in ts_data.get("pack_calc", []):
            ts_ms = int(p["ts"])
            val = p.get("value")
            if not isinstance(val, str):
                continue
            out = _extract_from_pack_calc(val, body.data_types)
            if out:
