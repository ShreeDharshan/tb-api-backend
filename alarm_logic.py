...
# Payload structure for alarm checking
...
# Internal caches (non-persistent)
device_cache = {}
bucket_counts = {}
door_open_timers = {}
device_door_state = {}

# Device ID lookup (cached)
...
def create_alarm_on_tb(device_name: str, alarm_type: str, ts: int, severity: str, details: dict, jwt_token: str):
    ...
    if not device_id:
        print(f"[ALARM] Could not fetch device ID for {device_name}")
        return
    alarm_payload = {
        "originator": {
            "entityType": "DEVICE",
    ...
    if 200 <= response.status_code < 300:
        print(f"[ALARM] Created: {alarm_payload}")
    else:
        print(f"[ALARM] Failed: {response.status_code} - {response.text}")

# Bucketed threshold check for jerk/vibration
...
def floor_mismatch_detected(height: float, current_floor_index: int, floor_boundaries_str: str) -> bool:
    try:
        ...
        print(f"[ERROR] Floor mismatch logic: {e}")
        return False

# Delayed door alarm trigger
def schedule_door_alarm(device_name: str, floor: str, ts: int, jwt_token: str):
    def fire_alarm():
        ...
        print(f"[DOOR] Door open too long alarm fired for {device_name}")
...
# === Main Entry Point for Alarm Triggering ===
