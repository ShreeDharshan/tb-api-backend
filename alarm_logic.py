...
THINGSBOARD_HOST = os.getenv("TB_HOST", "http://192.168.100.210:8080")
JWT_TOKEN = "Bearer <your_token_here>"
...
def get_device_id(device_name: str) -> Optional[str]:
    ...
    res = requests.get(url, headers={"X-Authorization": JWT_TOKEN})
...
def create_alarm_on_tb(device_name: str, alarm_type: str, ts: int, severity: str, details: dict):
    ...
    if not device_id:
        print(f" Could not fetch device ID for {device_name}")
        return
    alarm_payload = {
        "originator": {
            "entityType": "DEVICE",
    ...
    response = requests.post(
        ...
        headers={"X-Authorization": JWT_TOKEN, "Content-Type": "application/json"},
        json=alarm_payload
    )
...
def floor_mismatch_detected(height: float, current_floor_index: int, floor_boundaries_str: str) -> bool:
    try:
        ...
        print(f"Error in floor mismatch logic: {e}")
        return False
...
def schedule_door_alarm(device_name: str, floor: str, ts: int):
    def fire_alarm():
        ...
        print(f" Door open too long alarm fired for {device_name}")
